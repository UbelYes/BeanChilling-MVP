...
A Node.js Cloud Function was created in `gcp-proxy/` to act as a secure proxy between the browser and Vertex AI. Two files were created:

- `gcp-proxy/index.js` — the function logic
- `gcp-proxy/package.json` — dependencies (`@google-cloud/functions-framework`, `google-auth-library`)
...
The webapp is a single HTML file with a linked CSS and JS file. No build tools, no framework. This was a deliberate decision to:
...
[2] Browser reads file as ArrayBuffer (FileReader API)
        │
        ▼
[3] script.js stores raw Blob in memory (currentImageBlob)
...
[4] User clicks Analyze
        │
        ▼
[5] script.js reads Cloud Function URL and API Key from localStorage
        │
        ▼
